...
云函数默认拒绝代理访问内网/保留地址（`127.0.0.0/8`、`169.254.0.0/16`、`10.0.0.0/8` 等），防止函数被当作内网探测跳板。漏洞探测场景下 403 会被扫描器误判为"端口开放"，可在 `deploy.toml` 中关闭：

```toml
[security]
ssrf_protection = false   # 放行内网/保留地址，重跑 deploy.py 生效
```

无论开关状态如何，云平台元数据服务（`metadata.tencentyun.com` 及 link-local 元数据地址）始终被拦截，防止 Token 泄露后函数被用于窃取云凭证。
